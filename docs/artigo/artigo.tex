...
descreve um algoritmo consciente de esparsidade, para dados esparsos, e um
esquema aproximado de busca de pontos de corte baseado em quantis ponderados
(\emph{weighted quantile sketch}), além de otimizações de acesso a cache,
compressão e particionamento dos dados --- o conjunto dessas técnicas é o que
torna o método viável em bases com dezenas de milhões de linhas, como a nossa.
Nos experimentos deste trabalho usamos o modo \texttt{tree\_method = "hist"},
que é um mecanismo distinto, de discretização dos atributos em histogramas de
\emph{bins}, também voltado a bases grandes.
...
\textbf{Resultados esperados.} Dado o conjunto restrito de atributos, era
plausível esperar desempenho modesto em termos absolutos, mas claramente
...
O título do conjunto no portal menciona o intervalo 1979--2019, mas a página
disponibiliza também os arquivos anuais posteriores: os arquivos efetivamente
baixados e usados aqui cobrem 2000--2026, conforme registrado em
...
analógica. Ainda assim, os três critérios que sustentam a dispensa se aplicam
integralmente aqui: os dados são \emph{públicos}, \emph{secundários} (não
foram coletados pelo grupo, nem houve contato com participantes) e
\emph{anonimizados}, no sentido do art.~5º, inciso III, da
LGPD~\cite{lgpd}. Por esses critérios, e em linha com a orientação da
disciplina, o grupo conclui que, em princípio, a submissão ao CEP não é
necessária. Isso mudaria se o projeto passasse a usar registros
...
Forest (0,5363 contra 0,4975 do XGBoost). Esse padrão corresponde ao que era
plausível esperar (Seção~\ref{sec:metodologia}): idade e sexo quase determinam XV e
