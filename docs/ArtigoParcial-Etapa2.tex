...
\begin{itemize}
  \item Idade: decodificada para anos completos a partir do código do SIM (o primeiro dígito indica a unidade e os dois últimos, o valor).
  \item Escolaridade e raça/cor: mapeadas para categorias nomeadas, com ``Ignorado'' como categoria explícita em vez de imputação. Nenhuma linha é descartada por esse motivo: ``Ignorado'' soma 26,409\% em \texttt{ESC} e 5,389\% em \texttt{RACACOR}, e descartá-las enviesaria a amostra contra os anos iniciais. O código 0 de \texttt{ESC} não consta do dicionário do SIM e foi mapeado para ``Ignorado''; a decisão é discutível e está isolada em uma constante do código.
  \item Município: truncado para 6 dígitos e agregado em unidade da federação. Com mais de 5.500 municípios, o \emph{one-hot} seria inviável e o risco de reidentificação em células pequenas aumentaria (Seção~\ref{sec:etica}).
  \item Filtros: foram descartados 15.842 registros com \texttt{SEXO} ignorado, 81.308 com idade desconhecida e 3 sem capítulo da CID-10 (97.153 no total, 0,2977\% do bruto), restando 32.531.918 registros e 19 classes, sem valores nulos.
\end{itemize}
